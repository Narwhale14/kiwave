...
import { gainToDb } from '../util/audio';
...
<template>
  <div class="flex flex-col h-full w-full bg-mix-15">
    ...
    <div class="flex flex-row flex-1 overflow-x-auto min-w-0">
      <div v-for="mixer in mixers" :key="mixer.id" class="flex">
        <!-- each mixer -->
        <div class="flex flex-col w-28 items-center min-w-0 gap-1 px-2 hover:bg-mix-20 py-1 transition-colors shrink-0" :class="mixer.id !== 'master' ? 'border-r-3 border-mix-20' : ''" @pointerdown="activeMixerId = mixer.id">
          <!-- mixer name -->
        ...
        <span v-if="mixer.id === 'master'" class="w-2 bg-mix-5"/>
      </div>
